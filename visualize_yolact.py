# 用于mmdetection的yolact模型输出json文件的可视化
...
class visualize_mmdetection():
    def __init__(self, path):
        ...
        self.dict_list = list()
        #绘制loss
        self.loss_name = ['loss_cls', 'loss_bbox', 'loss_segm', 'loss_mask', 'loss']
    ...
    def plot_loss(self):
        ...
        #train
        i = 0
        plt.subplot(3, 4, 1, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        i = 1
        plt.subplot(3, 4, 5, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        i = 2
        plt.subplot(3, 4, 9, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        i = 3
        plt.subplot(3, 4, 3, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        i = 4
        plt.subplot(3, 4, 7, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        #val
        i = 0
        plt.subplot(3, 4, 2, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
        i = 1
        plt.subplot(3, 4, 6, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
        i = 2
        plt.subplot(3, 4, 10, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
        i = 3
        plt.subplot(3, 4, 4, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
        i = 4
        plt.subplot(3, 4, 8, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
...
if __name__ == '__main__':
    # 这个地方可能需要修改
    jsons_path = './work_dirs/yolact_r50_1x8_coco/*.json'
