...
class visualize_mmdetection():
    def __init__(self, path):
        ...
        self.dict_list = list()
        #绘制loss
        self.loss_name = ['loss_mask', 'loss_cls', 'loss']
    ...
    def plot_loss(self):
        ...
        #train
        i = 0
        plt.subplot(321, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        i = 1
        plt.subplot(323, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        i = 2
        plt.subplot(325, title='train_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.train_loss_list[i], 'b')
        #val
        i = 0
        plt.subplot(322, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
        i = 1
        plt.subplot(324, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
        i = 2
        plt.subplot(326, title='val_' + self.loss_name[i], ylabel='loss')
        plt.plot(self.epoch1, self.val_loss_list[i], 'g')
